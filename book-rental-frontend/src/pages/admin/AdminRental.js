...
import { searchUserRentalByName } from "../../service/Fetch.js";
import { rentalsActions } from "../../store/rentalsReducer.js";
...
const AdminRental = () => {
    ...
    const rentalsData = useSelector((state) => state.rentals.rental);
    ...
    const handlerDetail = (rental) => {
        history.push(`/admin/rental/detail/${rental.userId}`, { data: rental })
    }
    ...
    useEffect(() => {
        ...
        searchUserRentalByName(`admin/getuserrentalpagination/key=${keywordValue}?pageNo=${pagination.number}&pageSize=${pagination.size}`, jwtToken).then(res => {
            dispatch(rentalsActions.setRentals(res.data.content))
            setPagination({
                ...
                <div className="pagetitle">
                    ...
                    <nav>
                        <ol className="breadcrumb">
                            ...
                            <li className="breadcrumb-item active">Rental</li>
                        </ol>
                    </nav>
                </div>
                {/* <!-- End Page Title --> */}
                        ...
                        <div className="col-lg-12">
                            ...
                            <div className="card">
                                <div className="card-body">
                                    <h5 className="card-title">Rental Table</h5>
                                    ...
                                    <table className="table table-striped">
                                        <thead>
                                            <tr>
                                                ...
                                                <th scope="col">Day Status Rental</th>
                                                <th className="d-flex justify-content-end" scope="col">Action</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            {rentalsData.firstName !== null &&
                                                <>
                                                        ...
                                                        return (
                                                            <>
                                                                <tr>
                                                                    ...
                                                                    <td>{formatDate(rental.startRentalDate)}</td>
                                                                    <td className="ps-5">
                                                                        {dayStatus(rental.startRentalDate)} Days
                                                                    </td>
                                                                    <td className="d-flex justify-content-end">
                                                                        <button type="button" className="btn btn-info ms-1" onClick={() => handlerDetail(rental)}><i className="bi bi-reply-fill"></i></button>
                                                                    </td>
                                                                </tr>
                                                            </>
                                                        )
